...
\newcommand{\mjmtitle}{Copper Supplementation in Dogs: Listen to Her Heart Everybody's got a Hungry Heart }
...
\newcommand{\mjmbibdoi}{10.5281/zenodo.15785347}
...
\newcommand{\mjmaddr}{157 Zachary Drive Talking Rock GA 30175 GA }
...
.
In one case of simple copper deficiency with no obvious
causes first presented in May 2002
was not diganosed until December 2003 with authors concluding  
response to copper can be rapid avoiding inappropriate work up if recognized early
\cite{Klevay_Ux201c_char_2006}.
An overview by Brewer \cite{Brewer_Copper_toxicity_Alzheimer_2012}
 in 2012 was quite emphatic about copper and iron
supplements in older people, suggesting few people need them and they
can be dangerous particularly for cognition and recommended avoiding
meat.
However, a 2018 work suggested widespread copper deficiency
as a cause of many diseases currently common in North America
such as "obesity, ischemic heart disease, and metabolic syndrome" 
\cite{PMC5619695}.
A 2024 work recognizes copper distribution as a contributor
to atheroscloerosis and deficiency as a possible issue but
does falls back to chelators and ionophores as possible sultions
\cite{Yang_Li_Zhou_Copper_homeostasis_cuproptosis_2024}
not considering the possibility of global deficency leading
to bad allocation.
...
. 
\mjmtol{ Probably a lot of indirect observations also suggest a 
role in heart disease. 
For example,  MnSOD polymorphisms that correlate with "deficient cardiovascular
function"\cite{Azadmanesh_Slobodnik_Struble_Revealing_atomic_2024}
suggest the importance of mito ROS which may be generated with
copper deficiency. }


In dogs, both dietary and genetic issues are a known concern.
...
{
Its important to remember that all food components
are chemicals and have interactions beyond their
role such as a vitamin. As soon as they mix they can
react during preparation, storage, or consumption.
The importance of total meal  chemistry for copper
availability was recently made for poultry
\cite{Wu_Tan_Shi_vitro_bioaccessibility_2024}.
However, a couple of papers from 1964 illustrate the problem 
of reactivity quite clearly  as "copper" toxicity varied 
significantly with meal formulation.
Initially turkeys were fed either a natural or a purified
...
\cite{Waibel_Snetsinger_Ball_Variation_Tolerance_1964}.
A follow up work suggested the purified diet could be safe
with copper as it  considered vitamin A, D, and zinc 
additions and noted an off odor in high copper feed
\cite{Supplee_Observations_Effect_1964}
suggestive of undesirable reactions products . 
Its curious no one considers these results today.
The possibility of
vitamin depletion and formation of toxics,
notably anti-vitamins, needs to be considered in copper mixtures.
This is of course in addition to copper absorption efficiency.
Simple things can matter a lot for many reasons. 

Ideally the optimal fate of all consumed nutrients  
could be identified and manipulated  with  both meal and diet composition.
This work includes consideration of 
a background feeding context including real foods
and controlled supplements  that  has been described
previously
\cite{marchywka-MJM-2021-018-0.50rg}
. This diet was further refined by consideration of the interactions
described herein.
...
However, other nutrients have been observed to modify copper distribution.
...
A high calcium diet increased liver Cu in pigs 
\cite{Prince_Hays_Cromwell_Interactive_Effects_Dietary_1984}.
Further, copper reactions in stored food could damage some of these
nutrients hypothetically at least allow creation of  an association 
between pre-mix and accumulation but
mediated more by vitamin destruction than total copper consumption. 
Interestingly, although riboflavin is thought to be stable
under many conditions it may degrade with metal sulfates but
not chelates
\cite{Choe_Huang_Min_Chemical_Reactions_Stability_2005}.
It is possible to form anti-vitamins which may be potent inhibitors
of original vitamins but with passive non-saturating uptake.
\mjmtol{citation needed as they say on wikipedia ... }
At least one report indicated  that hepatic export of ceruloplasmin
was stimulated by thyroid hormone 
\cite{Mittag_Behrends_Nordstrom_Serum_copper_2012}
suggesting that hypothuroidism could allwo for hepatic copper
accumulation. 
As may be expected, copper sufficiency modifies metabolism
of other entities such as 
amino acids and in particular tryptophan. Copper deficient
rats appeared to metabolise tryptophan loads differently
and excrete amino acids but only taurine was found in the urine
of copper sufficient animals
\cite{Gray_Daniel_Effect_Copper_Status_1973}.
As amino acid status in old age may be a concern these effects
of copper may be of larger importance. 
...
%\mjmtol{ " the facts are in there " .. somwhere } 
...
If you only found the hoard, you may be tempted to chelate.
}
...
\newcommand{\mjmambig}
{
\mjmtol{now Cp comes up twice maybe condense and lead from chem
to clinic. Cu to cough a bit of leap  }
Location specific quantities can be the result of many
contributions and don't mean much without the "cause and effect"
leading to the result. 
\mjmtolx{
In any case, copper is used in several important locations and a reasonably
complete snapshot of copper distribution would 
be a vector. Easily accessible values such as blood levels
, generally dominated by acute phase protein ceruloplasmin,
are more responsive to acute pathological situations than
overall body total copper. 
}
Blood or serum levels are easy to obtain as part of normal
practice but copper's functions often execute in less accessible locations.
As details are often unknown, a colloquial
analogy may be trying to infer adequacy of military hardware
from measuring the amount on civilian roads. Blood vessels may be
used for transport in times of stress raising levels independent of
global  or niche amounts. 
There is a recurring problem using blood levels of some thing to indicate
excess or deficiency.  Some blood components such as electrolytes
may need to be closely regulated but this is not the case
when materials are just "passing through." These  blood components are
regulated or influenced by specific factors having nothing
...
Association studies may then find a correlation between high
blood copper and some bad state
as has been noted for cardiovascular 
\cite{Fox_Mazumder_Ehrenwald_Ceruloplasmin_cardiovascular_disease_2000}
 and Alzheimer's
\cite{Siotto_Simonelli_Pasqualetti_Association_Between_Serum_Ceruloplasmin_2016}
disease 
although a protective role now tends to be recognized
\cite{Liu_Wang_Zhang_Molecular_Functions_Ceruloplasmin_2022}
.
This work describes some important well known locations
that copper could exist although surprises are still likely.
Recently some body of work has suggested that macrophage
lysosomes may also regulate overall metal distribution
\cite{10.1039/c6mt00058d} a location that would be expected to
respond to many stimuli. Overall it appears that mitrochondria
and energy intensitve organs like the heart may be the most
important influencers. 

One meta analysis of bone loss and copper suggested only
...
\cite{PMC3691478}.
5mg was used to demonstrate an increase in human growth hormone in
6 of 12 tested subjects \cite{PMID3203160}.
This work describes supplementation to dogs with similar dose
ranges although much higher does normalized to body weight.
...
of copper probably makes a complete list intractable.
A starting point for a  'usual suspect" reaction participants list is 
in \mjmrefapp{interactions}
merely to indicate a lot of what is left out of the current analysis.
Interactions with sulfur will be discussed in more detail later as
these appear to be most important but others likely matter too.


Some of the issues include, 

\begin{enumerate}
\mjmecu{pH dependent solubility}{ An early low pH step such as in the stomach may be important for later absorption. }
\mjmecu{reduction}{ Copper is normally divalent but can be reduced leading to both more reaction products and precipitates. }
\mjmecu{insoluble precipitates}{Copper reacts with many things to form solids that may not be extractable from  GI tract. }
\mjmecu{destruction of vitamins}{ A big concern is creation of anti-vitamins which can be more potent than the vitamin being antagonized making it difficult to fix with more vitamins. These vitamins may be important to copper distribution and utilization leading indirectly to things like excess copper accumulation. }
\mjmecu{other reactions such as Fenton chemistry }{There is an ability to do a lot of oxidative damage. }
\mjmecu{competition with other metals}{ This is most commonly considered in uptake and enzyme metallization.
The copper transporter Ctr1 is thought to be selective against
most divalent metals as it transports Cu(I) but is inhibited
by Ag \cite{PMC3689948}. 
Enzymes may be subject to mis-metallization although
there may be evolutionary pressure to use a variety
to metals \cite{PMC8633580}.

 }

\end{enumerate}

Many of these may be a concern at each step of copper handling.

\mjmss{ Food Preparation}

\mjmresolve

\mjmss{ Intestine }
The initial important GI step is probably the stomach but
the function of the stomach has to be considered in terms
of achieving absorption in the intestines. 
Past the stomach into the intestines,  the pH increases
...
\mjmtol{cancer section? nutrient interactions here }
...
\mjmss{ The overall diet may be important for copper distribution  }
...
\newcommand{\mjmoverall}
{
The complete diet probably makes contributions to the overall distribution
of copper in the body and impacts ability to use copper beneficially.
This includes components consumed with or separately from copper.
As sulfur is one of the better studied and most important issues
...
\cite{PMID7825519}.
Once in the body, the ability to 
...
{
\mjmtol{ this probably is a work of its own like the manuscript
I started years ago although the application to copper is relevant. Maybe a bit grandiose and verbose too but this is important. Sentences longer than one paragarph? LOL.   }
...
no obvious harm. % harm with added copper.
...
supplements and avoid delays in understanding the
limitations of data, theory, and even anecdotes, to design 
therapeutic interventions.  In many cases, where different
types of information seem controdictory, it may turn out
that resulting  paradoxes are really regulatory paradigms.  

 Some types of problems
are listed in  \mjmrefapp{errors} as a general giuide. 
Due to the nature of the issue and time constraints, they 
are not well organizaed in any sense but may elicit an interest
in further consideration of existing data. 
\mjmtol{ this is rambling and vague maybe save the point for
coagulation, alzheimer's etc where it is more to the point.
Adding logic and orgainzation to reasons for illogic may not be 
worth it ...  also consider "too stupid" or " no one is that stupid" 
to highlight arrogance lol.  }
While it may help to make these things explicit by listing them
or naming them, one unifying theme is missing cause and effect
or system organization. Typical responses may try to disable 
a 'system" instead of fixing it leading to limited outcome quality.  
One common problem that may turn out to be relevant here
is a belief in opposites and concluding that if decreasing
some quantity helps then there is no benefit in increasing
under different conditions or states. In fact, many control loops appear
to "try harder" in response to initial deficiency  with 
goal related feedback creating inconsistent
associations between various quanitities.
This is most likely if something ( else )  doesn't work which may be due
to "quality" and feedback based on a goal or  result. Instead of disabling
the system, fixing it with more of what it needs is likely a better approach 
. In the case of copper,
local deficiencies simultaneous with other excesses may indicate a lack
of some other nutrient that is believed to be bad.  
Trying to treat quantities, or make a concentration similar to 
healthy people, may defeat an adaptive mechanism making things worse. 
...
entity.  
\mjmtol{ keep AD section as fact collection for now } 
% and AD  
...
Given all these revisions to cause and effect in mainstream AD literature,
...
% and unrelated coagulation, drop? 
...
The most well known issues with copper and dietary sulfur 
involve cows due to its common and profound economic impact
but related studies have been conducted in non-ruminant 
lab animals. The latter may be more directly relevant to
dogs and humans but the severity of the problem in cows
may indicate some effects important but more subtle in the target
population.  
In the other end of the spectrum may be   poultry, mostly turkey and chickens, which may be
notable for contrast to both mammals and ruminants due to
 the low pH proventriulus 
\cite{RILEY_AUSTIC_Influence_Dietary_Electrolytes_1984}
as well as the gizzard with low but maybe variable pH
\cite{Lee_Dunne_Mottram_Effect_diet_phase_2017}
.
Experiments on non-ruminants cover some of the supplements
used in this work that could impact copper sufficiency and
toxicity.
\mjmtolx{
...
\mjmtol{ repeated text / citations in  discussion }
...
Ascorbic acid in rats initially reduced absorption of
copper but may eventually limit excretion \cite{VandenBerg1994}.
...
Pig tolerance of sulfur appears to be significant as higher
sulfur diets did not seem to impact
measures of copper  such as liver content 
\cite{Kim_Kil_Mahan_Effects_dietary_sulfur_2014}
although another work showed more complicated results
with reduction  in ceruloplasmin being one result  
\cite{Dale_Ewan_Speer_Copper_Molybdenum_Sulfate_1973}.
The latter work noted decreased liver copper with the addition
of potassium carbonate. 

Exploration of optimial copper supplementation in poultry
...
%\newcommand{\mjmhappymisc}{ \begin{figure}[htb] \centering \mjmfullplot{happy_misc.jpg} \caption{   Copper shown for daily and 10 day trailing average along with chicken thighs ( magenta) as a measure of fat intake. Recovery from low fat diet appears to take longer than recovery from only copper deficiency.  } \end{figure} } % mjmhappymisc
...
\begin{figure}[htb] 
\centering
%\mjmfullplot{happy_cu30.jpg}
\mjmfullplot{happy_cuznall.jpg}

\caption{ Copper( yellow ) and Zinc( white) 30 day trailing average for
Happy. }
%\vspace{.5in}

%\mjmfullplot{happy2seeyou.jpg}
%\caption{  
%Above, 30 day trailing average daily copper intake over entire time here.  Copper(white) and Zinc(yellow) dosing per day averaged over prior 10 day period as dosing was highly variable due to rotations of various nutrients.  18046 is 2019-05-30 when the cough was first noted to be gone for a few weeks. 19823 is  2024-04-10 the last date for which data was obtained. The cough stopped prior to the start of the Zinc and gradually increased to a notable background level over most of this interval although notes were incomplete.  19531 2023-06-23 notes the start of Cu depletion and chronic cough was noted by late Fall. During this time Zinc greatly exceeded copper dosing.  } 

\end{figure}

%\begin{figure}[htb] \centering \mjmfullplot{happy_recent.jpg} \caption{  Most recent with some passing progress   } \end{figure} \begin{figure}[htb] \centering \mjmfullplot{happy_early.jpg} \caption{  Earlier when she was cough free for a whlie   } \end{figure}


\begin{figure}[htb] 
\centering
\mjmfullplot{happy_cuzn.jpg}
\caption{  
30 day trailing average for Happy copper(yellow) and zinc(white).
2019-05-30 (18046)  when the cough was first noted to be gone for a few weeks. 
In retrospect decreasing copper and the start of zinc should have been significant concerns. 
   }
\end{figure}


%\begin{figure}[htb] \centering \mjmfullplot{happy_citrate.jpg} \caption{  Post script switch to Chinese citrate and increase dosing to no obvious benefit   } \end{figure} 
...
\begin{figure}[htb] 
\centering
\mjmfullplot{trixie_cu30.jpg}

\vspace{.25in}

\mjmfullplot{trixie_cu.jpg}
\caption{   Trixie copper consumption since arrival. Top plot 30 day trailing average per day. Daily amounts
( white)) and trailing 10 day average( yellow).n bottom  Copper started to
...
\begin{figure}[htb] 
\centering
\mjmfullplot{rocky_cu30.jpg}

\vspace{.25in}

\mjmfullplot{rocky_o30.jpg}

\caption{    Rocky 30 day trailing average copper (above  ) and iodine (below)  .  Its probably important to note that iodine was only given every few days and this much continuous iodine may have different net effects. This average is just included to illustrate correlations with a likely confounding factor.    }
\end{figure}
...
\newcommand{\mjmotherscu}{
\begin{figure}[htb] 
\centering
\mjmfullplot{daisy_cu.jpg}


\caption{     Daisy monthly copper/day averages }
\end{figure}

\begin{figure}[htb] 
\centering

\mjmfullplot{claire_cu.jpg}


\caption{      Claire monthly copper/day averages }
\end{figure}

\begin{figure}[htb] 
\centering
\mjmfullplot{frankie_cu.jpg}
\caption{     Frankie monthly copper/day averages }
\end{figure}
...
\begin{figure}[htb] 
\centering
\mjmfullplot{dexter_cu30.jpg}
\caption{    Dexter daily copper intake 
since arrival 30 day trailing average.   
}
\end{figure}

\vspace{.2in} %keep/dexter_cuzoom.jpg

\begin{figure}[htb] 
\centering
\mjmfullplot{dexter_cuzoom.jpg}
\caption{    Dexter daily copper intake around blood test on day 19926 
.  Daily amount in red 10 day trailing 30 day average in white.
While his consumption had recently been higher than normal he had not
had any for about 18 days prior to blood test.
  }
\label{fig:dexzinc}
\end{figure}
...
\begin{figure}[htb] 
\centering
\mjmfullplot{dexter_zn.jpg}
\caption{      30 day trailing moving average zinc intake for Dexter.
Interestingly he seems to have gained weight after the peaks and the
null was prior to his vet visit. It is likely he is sensitive to zinc
and does seem to be responding well to the occaisonal "DEXSNACK"
containing several non-copper metals.
  }
\end{figure}
...
\mjmfullplot{brownie_cu30.jpg}
\caption{    Brownie 30 day trailing average copper.  }
\end{figure}
} % mjmrowniecu
...
\begin{figure}[htb] 
\centering
\mjmfullplot{annie_cu30.jpg}
\caption{    Annie overall 30 day lagging average daily Cu (mg).  }
\end{figure}
...
Alzheimer's & human  &brain depletion with excess Cp \cite{Cooper_Xu_Church_Evidence_widespread_severe_2017} &&  \\ 
Infection & human  & && \\ 
...
Hershey  &17-04-22 19-08-27&multiple&8.2 -  9 &.2-.6&.1& fatal heart failure  \\
\hline
Happy  &18-09-07 24-04-10&several&13.4 - 17.7&&& \\
Happy  &18-09-07 19-05-30&heartworm/doxycycline&13.4 &2&.29&cough gone briefly  \\
...
Annie  &2022-09-21 &excessive sleep, apathy &7.74- 9.9  &2 & .5 & briefly active again   \\
Dexter  &2017-04-25 &picky eating  &5 - 6.8  & variable low  & "low"   & see text   \\
\hline
\multicolumn{7}{|c|}{ dogs given Cu but not detailed } \\
\hline
Claire &\multicolumn{1}{|l|}{ 2018-07-08} &none &&see data && none \\
Frankie &\multicolumn{1}{|l|}{ 2019-10-12 } &none&&see data&& none, aka "Autumn"  \\
Daisy &\multicolumn{1}{|l|}{ 2023-03-28} & UTI early on  &&see data && none \\
\hline
\hline
\end{tabular}
...
In 2025, one group has improved bioprosthetic heart valves
by addinc copper dopped carbon dots\cite{PMID40424007}.
...
identified for causing or preventing it. 
Recently in humans, the risk of recurrence of specific 
fibrillation or heart failure types was shown to segregate
according to 3 nutritional indexes with HR's in the 10-20
range \cite{PMC11785320} suggesting general measures can already
have value  and a common nutritional origin that may include several
components.
...
Copper deficiency is known to produce cardiac hypertrophy
in animals along with a large number of mitochondria
that may be adapted to the low copper state 
\cite{Medeiros2002}.
...
The role of copper in mammal interaction with pathogens
has been well reviewed in works such as \cite{PMC9302775}.
Effects of copper on infection were known since at least 
2400BC  with other notable observations such as 
benefits to copper workers against cholera in the 1800's

and today with interest in copper surfaces 
\cite{Samanovic_Ding_Thiele_Copper_Microbial_Pathogenesis_2012}
...
Copper was shown to reduce Proteus mirabilis colonization
of mouce urinary tract with decreased ureas and secretion and
enhanced macrophage killing although with increased
biofilm formation and resistance to antibiotics
\cite{Huang_Lee_Wang_Copper_affects_virulence_2024}.
This delay but ultimately grow stonger effect was also
seen in cultures of Bordetella  pertussis although
the authors consider it "misleading" as the
effect appears to be due to the relative Cu(i) and
Cu(ii) concentrations created by ascorbate in the
grwoth medium 
\cite{RiveraMillot_Slupek_Chatagnon_Streamlined_copper_defenses_make_2021}.
Another work with Pseudomonas fluorescens SBW25 and growth
on copper surface demonstrated adaptive mutations
but reduced vigor and biofilm formation  in plain broth 
although with increased antibiotic resistance
\cite{Xu_Liu_Naren_Experimental_evolution_bacterial_2022}.
If the organisms are signoficantly less fit in other environments
the adaptations may not matter as the organisms could die out
after copper exposure. 



Bacterial copper absorption is thought to passive in many cases
...
Several works find decreased serum copper as a correlate
of thyroid  hormone levels 
\cite{Blasig_Kuhnen_Schuette_Positive_correlation_thyroid_2016}
\cite{Athokpham_Mohanty_Pinnelli_Status_Serum_Calcium_2020}
\cite{Kim_Kim_Chung_Exploring_role_2020}
while some find no correlation
\cite{vv304hamAljuboori_Alrawi_Ahakeim_Estimation_Serum_Copper_2010}
\cite{Ali_AbdulHussein_Tahssen_Estimation_Serum_Copper_2019}
but relevance is unclear as this may just relfect
low amounts of ceruloplasmin which is an acute phase
protein. 

Hypothyroidism may seem out of place but it is a common
problem in dogs and as it involves lethargy may be confused
with unrelated energy issues such as the mitochondiria considerations
also discussed herein. Interesting literature on copper in poulty
exists largely due to thyroid effects and anti-nutrients
in common feeds. For example, canola meal contains
tannins, glucosinolates, and phytic acid with broiler
performance and thyroid parameters shown to improve
 with the addition
of substantial copper sulfate to this feed 
\cite{AzimiYouvalari_Farhoomand_BaghbanKanani_Copper_sulfate_arginine_2022}
. This work also briefly mentions ascites and thyroid function 
which is interesting due to one of the heartworm dogs here getting
huge fluid accumulation during recovery ( unpublished observatrions ).   
Similarly, pigs fed on 8 percent rapeseed meal benefited from both
copper sulfate and iodine addition and anti-nutrients isothiocyanate and
oxazolidinethione were not longer detectable in the feed with copper
sulfate 
\cite{Ludke_Schone_Copper_iodine_1988}
indicating copper could destroy some anti-vitamins as well as vitamins.
Similar nutrients compared to HCl treatment for mustard meal
demonstrated similar benefits could be obtained 
in calves 
\cite{Tripathi_Agrawal_Sharma_Effect_untreated_2001}.
...
Copper supplements may compensate for marginal deficiencies
of related enzymes such as in mice heterozygous for functioing
peptidylglycine alpha-amidating monooxygenase
\cite{BousquetMoore_Ma_Nillni_Reversal_Physiological_Deficits_2009}
 . 
In another section the existence of functional copper relevant 
enzyme deficiency due
to amino acid deficiency is discussed. 
...
\mjmtol{ fact collection, keep analysis in "confusion" section I guess } 

The role of metal distribution in neurodegenerative diseases
is a topic of current interest with many
unresolved issues 
\cite{PMC11788444}.
...
Copper seems to be imporant to stem cell maintenance 
\cite{PMC11886605}.
...
High energy consumers such as the brain and heart appear 
to be significant in regulating copper distribution presumably
largely due to a need to nsure adequate mitochondrial ATP production. 
Within the mitochondria, copper is used in cytochrome C oxidase but
more generally complex IV status may be what is sensed to signal
copper status. While this must remain as speculation for now, this section
considers some features of complex IV that could modulate the overall
effectiveness of cytochrome C oxidase.   While perhaps strictly 
synonymous, the complex term will be used to include consideration
of cytochrome c and the overall environment. 
Besides copper, one possible problem in old age is high-infidelity
translation that could effect cytochrome C and the oxidase.
If these become dysfunctional that could confuse copper signalling.
...
CoX
\cite{Cooper_Xu_Church_Evidence_widespread_severe_2017}.
\mjmtol{ 3 cites for this work lol }
...
As early as 1950, it was noted that polyphosphates could
...
\mjmblackboxno{Release Notes  2025-07-01 : }{
I finally got this to into a state that highlights some important
points about this topic but don't think I'll have a lot of time to
clean it up and organize it and in any case I'd like to wait for
more data but that could take a while. 
...
beneficial. Often the popular press led by science
...
copper evolve. Curious for example about rhetoric related to PEX replacing
copper plumbing lol.  
I took some of the text from a much older manuscript, probably going
back to 20018 or so and it still seems aropos. 

It may look like introduction and discussion have been mixed up. 
...
rather simple work of designing a set of supplements for a group
of dogs.  One issue is "weighing" evidence about copper from different
types of sources. "Ancient civilizations" are introduced almost
at the end as a thought provoking topic for better context on possible
health effects of widespread copper exposure. 
...
\begin{abstract}
Copper is  important for diverse biological processes
such as energy production
and maintenance of cartilage quality as well as control of pathological
conditions such as infection and probably cancer. 
However, it  is not currently a trendy supplement for dogs or
humans ( usually omited from lists such as
most popular supplements \cite{Network_Supplements_} ). Part of the concern with dogs is the variable
...
organ specific  demand as part of the distribution control 
algorithm.  Such a system may malfunction to produce bottlenecks leading to
uncommonly large accumulation in an organ such as the liver
while another organ such as the heart is starved.
Such a situation could occur due to some other nutrient
limitation that fools or cripples  the feedback mechanisms.
...
accumulating organ( and this dichotomy assumed monotonic increasing totals with intake which need not be the case).  
A better solution is to find the 
pathways needed to match copper supply and demand 
and insure they are not limited by easily fixed deficiencies.
This work describes copper supplementation to a group
of dogs including the larger context of more careful supplementation.
No obvious significant harms were observed  while 
in some cases coinciding benefits such as
...
Results are considered in view of the human and dog medical literature but
also from diverse sources including
ruminent  and mining copper literature for a more complete understanding. 
Important considerations for copper supplementation
may include its reactivity, simultaneous meal components
such as sulfur containing molecules including taurine, overall average diet
composition, and
GI tract details. Copper has been observed to destroy both
vitamins and anti-vitamins suggesting a need to minimize contact with
vitamins or at least control the interaction.
This is important as "anti-vitamins" may be potent antagonists
rather than just lost vitamins. 
Strategies considered include separate feedings, time release formulations, 
and  use of specific forms such as copper citrate or glycinate.  
The absence of an early low pH step in digestion may allow
idiosyncratic formation of insoluble copper compounds
preventing uptake of copper and other vitamins pointing
to a possible benefit of acid or chloride supplemented diets especially
in the elderly.
Specific less obvious dietary components may %be of particular interest
be suspects in conditions such as canine DCM 
as they may limit copper delivery to the heart. 
%discussed here as an important issue in dogs and humans. 
Some common patterns of confusion seem to turn paradigms
into paradoxes making it difficult to formulate interventions
in medical conditions. With copper like in other cases many
of these relate to incorrect interpretation of associations
that result from complex feedback systems adapting to various
perturbations. Some of the more common fallacies are catagorized.
In particular, identification of "larger than normal" accmulation
of copper points to a common problem of trying to fix a quantity
regardless of what is causing that. This work will try to highlight
the supremacy of cause and effect over treating numbers. 


\mjmtolx{
Much of the confusion probably relates to digestive details
and overall diet composition even with similar copper handling
genetics. 
Copper is quite versatile and has been 
observed to destroy both vitamins and anti-vitamins.
In general diets it may be beneficial to separate copper
intake or use time release or citrate supplements.
The ability of specific diets to become pathological with 
copper inclusion is discussed at various points. 
Specific issues may induce copper deficiency such as many
dietary sulfur sources and notably taurine. This may be
made worse by the absence of a low pH step early in
digestion allowing idiosyncratic formation of insoluble copper compounds
possibly taking other vitamins away too. The pH-copper
relationship is not clear but its reasonable to consider
acid increasing diets especially for the elderly.  
} % mjmtol
...
\mjmtol{ this applies to other stuff but probably not to copper }
...
\begin{comment}
% this may be relevant but too much text already 
...
\mjmguide{ Copper related outcomes depend on everything starting with food preparation   }
...
While I was gone he went to the vet for vomiting. His recent copper
intake over the last few months had been much higher than normal
but he had not eaten any copper since 2024-07-04(19908), 18 days prior,
 and was not exhibiting
any signs inconsistent with his normal pickiness. 
Blood work on 24-07-22( 19926) 
...
His zinc history, see \mjmreffig{dexzinc}  does show lack of supplementation prior to this
event and weight gains occur  after more prolonged higher supplementation
. 
Rats have been observed to have similarly reduced food intake with
zinc deficiency as well as bone abnormalities
\cite{PMC2882585}.
...
%His vet visit was on 2024-07-22 with last recorded copper 
%intake of .4mg on 2024-07-04 and on 2024-07-03. 
Other instances of spitting up were recorded inconsistently.
He was noted
...
Currently he seems to be gaining weight and eating more consistently
with possible improvement after the switch to cooked beef
adding ciric acid and KCl to his diet. Chloride may be important
as well as the non-copper metals. 
} % mjmdextertext

\newcommand{\mjmcookietext}
{
...
\cite{mmarchywka-MJM-2021-003-v0.50rg}. 
She was notable for one bad eye and abdominal lump.
Briefly, 
Brownie was determined to be pregnant shortly after arrival
.  She gave birth on 2021-02-14 ( day 18672) to 10 live puppies
and 2 dead or died shortly after delivery. One died later after apparently
being sat on.  
Her  heartworm
was  treated after weaning with Diroban on 
2021-07-13 and 14 ( days 18821 and 18822) 
.
She was then spayed on 2021-09-13 ( 18883 )  but suffered complications
requiring emergency intervention to drain fluid. 
Subsequent to that work, her abdominal lump 
was removed and diagnosed  by vet as fibroids 
on  2021-11-15 ( 18946 ) . She was
uneventful until she was observed to 
cough occasionally for no obvious reason and appeared to
have some movement issues. X-ray confirmed tumors were likely
responsible for both problems.   The vet suggested steroids
which may have caused a variety of problems and made it impossible
to encourage immune stimulation in any meaningful way and
probably not much could be done with added copper. 
She becamse agitated  and appeared to have a hard time
navigating as if her bad eye had been giving her some
information earlier.  
She was put to sleep on 2023-02-22 ( 19410 ). 
She never received high copper dosages  but given her
...
as she received doxycycline and ivermectin 
with vitamin K and other supplements
\cite{mmarchywka-MJM-2019-001-.1li}.
Later she had an unusual episode which resulted in some
...
and blood work. On 2022-02-02, after apparently choking on a carrot,
 lab resuls were notable for
...
On 2024-10-04, after having difficulty jumping,
 at a different clinic glucose was normal
...
\mjmtol{ not enough context to be useful here. Add or remove?}
For comparison, Little Man on 2016-12-29 while on steroids
IIRC had elevated ALP ( 231 U/L, 0-140 ) and high
glucose (126ml/dL, 75-124) and low BUN ( 8.6 mg/dl, 9-29).
ALP may have been elevated due to steroids rather than  copper
toxicity. 
\mjmtolx{ 
{\bf vague and redundant with prior paragraph }
...
been high in the past.  ALP was numerically similar
however and the difference in tests is unclear.
These results suggest no copper
...
feeling that seemeed to recur after one bath with special
shampoo ( don't remember if it was prescription check notes ) .  
...
days ( I was gone ) and owner took her to the vet as she
...
She had a renewed cough around 2024-05-22 (19865)  subsequent to
...
Her coat continues to  feel clean although difficult to determine
if bathing alone would have achieved same result.  

More recently she has been getting more complete supplementation
...
decreased but still common although her attention
level continues to increase and right now the cough is thought
to be tolerable.

After another round of antibiotics in response to 
coughing and low energy , she continues with increased
copper, lipoicacid, and histidine along with intermittent iodine. 
The histidine may increase her 
alertness but also possibly aggressivness. Engagement with
surroundings on walk improves although cough still comes and goes.
Daisy tries to play with her but she has limited interst. 


} % mjmtrixietext


\newcommand{\mjmrockytext}{
Rocky arrived here on 2022-02-05 in generally good condition
although notable for a "rubber block" or "plastic" body type or feel.
...
the other dogs when picked up rather than being stiff.
...
Benzoate was included in snacks before and notably in the
case of Beauty \cite{marchywka-MJM-2022-013-0.10} for a variety
...
He eats a lot more vigorously and consistently than in the past
although he also may have benefited from added iron.

} % \mjmrockytext

\newcommand{\mjmannietext}{
...
and seemed restored to her more typical self by
 2024-05-05 ( 19848 ) with continuing improvements in 
energy and appetite.
 % of 2mg cu. 
...
As of January 2025, she was slowly deteriorating with apparent
decreases in eyesight and awareness. Up until a week or so ago
she had been eating reliably but may have had too much zinc.

In mid April 2025, she is fairly stable eating well most of the
time with some episodes of not eating. She wanders around bumping
into things and sleeps a lot sometimes barking when everyone else is
barking. 
...
in good detail,  many caveats exist, see \mjmreftab{caveats}.
\input{muqed_caveats.tex}
...
days after copper intake was increased although she
does continue to slowly deteriorate.  
...
\mjmtol{ maybe add Daisy as she had earlier infection }
...
\input{ancient_copper.tex}



\mjmtolx{
Unlike many works, this effort tried to create a relevant
real-world context around the supplementation of copper.
Vitamin and minerals and indeed foods are complex chemicals
capable of many reactions and its difficult to isolate
the effect of any single one on a "typical" dog or person.
This work tried to "take the best shot" and optimizing
intake given suspected cause and effect.  } 



\section{Conclusions}

\mjmtol{ replace the madlib conclusions, repeat the intro lol,
with more alapoint darlings. Contrast engineering approach of "trying to do something better" with academic study.  }
...
but "we have computers" \mjmtol{ Life During Wartime lol } .  
...
\mjmtolx{
While "root causes" are a trendy topic, it may be worth noting
that you can keep chasing cause and effect indefinitely
but the point is to find the most beneficial intervention.
In cases where supplemental copper is required for optimal health,
there still may be some other defect in say the GI tract.
Copper supplementation may fix one problem but other nutrient
deficiencies may still remain and these others could impact
ability to use copper well. 
}
...
\mjmtol{ oh my darlings, integrate the zingers better lol }
...
\myitem{quantity may be due to a quality problem. }{ The feedback that produces inducible effectors may not go away until the job is done. If the effectors dont' work you will get more of them. Getting rid of them will not fix the original problem that may be important if this control system evolved well lol.  }
...
\myitem{a ratio is not a derivative }{ Associaion or fit parameters relating X to Y are averaged over an entire data set but the derivative at current state is what would be relevant to an intervention.   }
